...
//    mBandColors.append(pair);
    //Set white step
//    int b_Step;
//    qDebug() << "ini " << b_maxValue << b_minValuel << b_divValue;
//    b_Step = (double)(b_maxValue - b_minValuel)/(b_divValue * b_maxValue) * 100;
//    if(b_divValue != 0){
//        qDebug() <<"step " <<b_Step;

//        for(int i = 0; i < 100; i = i + b_Step){
//            pair.first = Qt::white;
//            pair.second = i + 1;
//            mBandColors.append(pair);
//        }
...
void QcColorBand::SetRangeOfColor(int minValue, int maxValue, int lowValue, int highValue, int divValue)
{
    QColor tmpColor;
    tmpColor.setAlphaF(0.1);
    QPair<QColor,float> pair;
    ...
    int b_Step;
    b_Step = (double)(maxValue - minValue)/(divValue * maxValue) * 100;
    if(divValue != 0){
        int lowPercent = ((double)(lowValue) / maxValue) * 100;
        int highPercent = ((double)(highValue) / maxValue) * 100;

        pair.first = Qt::white;
        pair.second = 1;
        mBandColors.append(pair);

        for(int i = 1; i < lowPercent + 1; i++){
            if(i % b_Step == 0){
                pair.first = QColor(0,102,255);
                pair.second = i;
                mBandColors.append(pair);
        ...
        for(int i = lowPercent; i < highPercent + 1;i++){
                if(i % b_Step == 0){
                    pair.first = Qt::black;
                    pair.second = i;
                    mBandColors.append(pair);
...
void QcColorBand::
setColors(const QList<QPair<QColor, float> > &colors)
{
    mBandColors = colors;
    update();
}
